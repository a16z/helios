...
use helios_mantle::config::{Config, Network, NetworkConfig};
use helios_mantle::spec::Mantle;
use helios_mantle::MantleClientBuilder;
...
pub struct MantleClient {
    inner: helios_mantle::MantleClient,
    chain_id: u64,
    active_subscriptions: HashMap<String, Subscription<Mantle>>,
}
...
impl MantleClient {
    ...
    pub fn new(
        ...
        network: String,
    ) -> Result<MantleClient, JsError> {
        console_error_panic_hook::set_once();

        let network_config = match network.as_str() {
            "mantle" => NetworkConfig::from(Network::MantleMainnet),
            "mantle-sepolia" => NetworkConfig::from(Network::MantleSepolia),
            other => Err(JsError::new(&format!("invalid network: {other}")))?,
        };

        let chain_id = network_config.chain.chain_id;

        // Use explicitly provided consensus_rpc since Mantle doesn't have a default one yet
        ...
        let inner = map_err(MantleClientBuilder::new().config(config).build())?;
    ...
    pub async fn subscribe(
        ...
        let subscription = Subscription::<Mantle>::new(id.clone());
    ...
    pub fn set_helios_events(&mut self, _handler: Function) -> Result<(), JsError> {
        // No events have been implemented for Mantle yet
        Ok(())
    }

    #[wasm_bindgen]
    pub async fn get_current_checkpoint(&self) -> Result<JsValue, JsError> {
        // Mantle does not support checkpoints
        Err(JsError::new("Mantle does not support checkpoints"))
    }
}
