...
Helios also provides documentation of its supported RPC methods in the [rpc.md](_media/rpc.md) file.
...
A comprehensive breakdown of config options is available in the [config.md](_media/config.md) file.
...
To run all benchmarks, you can use `cargo bench`. To run a specific benchmark, you can use `cargo bench --bench <name>`, where `<name>` is one of the benchmarks defined in the [Cargo.toml](_media/Cargo.toml) file under a `[[bench]]` section.

To learn more about [helios](https://github.com/a16z/helios) benchmarking and to view benchmark flamegraphs, view the [benchmark readme](_media/README.md).
