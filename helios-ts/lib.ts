import initWasm, { EthereumClient, OpStackClient } from './pkg/index.js'
...
export class HeliosProvider {
  ...
  constructor(config: Config, kind: 'ethereum' | 'opstack') {
    if (kind === 'ethereum') {
      ...
      this.#client = new EthereumClient(executionRpc, consensusRpc, network, checkpoint, dbType)
    } else if (kind === 'opstack') {
      const executionRpc = config.executionRpc
      const network = config.network

      this.#client = new OpStackClient(executionRpc, network as Network)
    } else {
  ...
  async #req(req: Request): Promise<any> {
    switch (req.method) {
      ...
      case 'eth_getTransactionByHash': {
        const tx = await this.#client.get_transaction_by_hash(req.params[0])
        return mapToObj(tx)
      }
