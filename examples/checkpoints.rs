...
async fn main() -> Result<()> {
    ...
    // The `build` method will fetch a list of [CheckpointFallbackService]s from a
    // community-mainained list by ethPandaOps. This list is NOT guaranteed to be secure, but is
    // provided in good faith. The raw list can be found here: https://github.com/ethpandaops/checkpoint-sync-health-checks/blob/master/_data/endpoints.yaml
    ...
    // Since we built the checkpoint fallback services, we can also just get the raw checkpoint
    // fallback services. The `get_fallback_services` method returns a reference to the internal
    // list of CheckpointFallbackService objects for the given network.
