...
use crate::evm::MantleEvm;

/// Mantle network spec.
///
/// Mantle is based on OP Stack but with customizations such as:
/// - MNT as the native gas token (instead of ETH)
/// - Preconfirmation logic on the sequencer
/// - EigenDA as the data availability layer
#[derive(Clone, Copy, Debug)]
pub struct Mantle;

#[cfg_attr(not(target_arch = "wasm32"), async_trait)]
#[cfg_attr(target_arch = "wasm32", async_trait(?Send))]
impl NetworkSpec for Mantle {
    ...
    async fn transact<E: ExecutionProvider<Self>>(
        ...
        let mut evm = MantleEvm::new(execution, chain_id, fork_schedule, block_id);

        evm.transact_inner(tx, validate_tx, state_overrides).await
    }
}

impl Network for Mantle {
...
impl TransactionBuilder<Mantle> for OpTransactionRequest {
    ...
    fn build_unsigned(self) -> BuildResult<OpTypedTransaction, Mantle> {
    ...
    async fn build<W: NetworkWallet<Mantle>>(
        self,
        wallet: &W,
    ) -> Result<<Mantle as Network>::TxEnvelope, TransactionBuilderError<Mantle>> {
