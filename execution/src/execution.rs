...
impl<R: ExecutionRpc> ExecutionClient<R> {
    pub fn new(
        ...
        let blocks = Blocks::new(block_recv, finalized_block_recv);

        Ok(ExecutionClient { rpc, blocks })
    }
    ...
    pub async fn get_block(&self, tag: BlockTag) -> Result<Block<Transaction>> {
        Ok(self
            ...
            .get_block(tag)
            .await
            .ok_or(BlockNotFoundError::new(BlockTag::Latest))?)
    }
    ...
    pub async fn get_account(
        ...
        let proof = self.rpc.get_proof(address, slots, number).await?;
        ...
        let code = if proof.code_hash == KECCAK_EMPTY {
            ...
            let code = self.rpc.get_code(address, number).await?;
    ...
    pub async fn get_transaction_by_block_hash_and_index(
        ...
        let block = self.get_block(block).await?;
        let tx = block.transactions.get(index).cloned();
    ...
    pub async fn get_transaction_receipt(
        ...
        let block = self.get_block(BlockTag::Number(block_number)).await;
        ...
        let block_receipts_root = block.receipts_root;
        ...
        let receipts_fut = tx_hashes.iter().map(|hash| async {
        ...
        let expected_receipts_root = ordered_trie_root(receipts_encoded);
        let expected_receipts_root = H256::from_slice(&expected_receipts_root.to_fixed_bytes());

        if expected_receipts_root != block_receipts_root || !receipts.contains(&receipt) {
