...
use alloy::{
    ...
    rpc::types::{AccessListResult, Block, EIP1186StorageProof, Header},
};
...
pub struct Evm {
    execution: Arc<dyn ExecutionSpec<OpStack>>,
    chain_id: u64,
    tag: BlockTag,
    fork_schedule: ForkSchedule,
}
...
impl Evm {
    ...
    pub async fn call(&mut self, tx: &OpTransactionRequest) -> Result<Bytes, EvmError> {
    ...
    pub async fn estimate_gas(&mut self, tx: &OpTransactionRequest) -> Result<u64, EvmError> {
    ...
    async fn get_env(&self, tx: &OpTransactionRequest, tag: BlockTag, validate_tx: bool) -> Env {
        let block = self
            .execution
    ...
    fn tx_env(tx: &OpTransactionRequest) -> TxEnv {
        TxEnv {
            ...
            gas_price: <OpTransactionRequest as TransactionBuilder<OpStack>>::gas_price(tx)
                .map(U256::from)
                .unwrap_or_default(),
            transact_to: <OpTransactionRequest as TransactionBuilder<OpStack>>::kind(tx)
                .unwrap_or_default(),
