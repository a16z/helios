#[tokio::test]
async fn rejects_account_proof_for_different_address() {
    ...
    let forged = provider
        .get_account(Address::ZERO, &[], false, alloy::eips::BlockId::latest())
        .await;
    handle.stop().unwrap();
    assert!(forged.is_err(), "accepted a proof for another account");
}
